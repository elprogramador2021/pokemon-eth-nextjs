import { Modal, SvgEdit, Typography, Input, Select } from "web3uikit";
...
export default function ModalType({ mostrarModal }) {
    ...
    //createTypesToWeaknesses
    const {
        runContractFunction: createTypesToWeaknesses,
        isLoading,
        isFetching,
    } = useWeb3Contract({
        abi: abi,
        contractAddress: pokemonFactoryAddress,
        functionName: "createTypesToWeaknesses",
        params: { _type: type, _weaknesses: weaknesses },
    });
    ...
    */
    ...
    const guardarArrayWeaknesses = (cadena) => {
        setWeaknesses(cadena.split(","));
    };

    const LimpiarCampos = () => {
        //const allTypes = (await getAllTypes()).toString();
        //console.log("3", allTypes);
        //Limpiar Campos
        setType("");
        setWeaknesses([]);
    };
            ...
            <div>
                ...
                >
                    {isLoading || isFetching || isLoadingSuccess ? (
                        ...
                        >
                            <div className="py-5">
                                <Input label="TYPE" width="100%" onChange={(e) => setType(e.target.value)} value={type} />
                            </div>
                            <div className="py-5">
                                <Input label="WEAKNESSES" width="100%" description="separated by commas" onChange={(e) => guardarArrayWeaknesses(e.target.value)} value={weaknesses.toString()} />
                            </div>
                        </div>
                    )}
                </Modal>
